...
public class UnifiedArmSubsystem extends BaseSetpointSubsystem<XYPair> implements DataFrameRefreshable {
    ...
    protected boolean areBrakesEngaged;
    ...
    public UnifiedArmSubsystem(
        ...
        // Setup WPI "Mechanism" for easy viewing of the arm on a dashboard
    ...
    public void periodic() {
        ...
        Logger.getInstance().recordOutput(getPrefix() + "AreBrakesEngaged", areBrakesEngaged);
        Logger.getInstance().recordOutput(getPrefix() + "CurrentXPosition", currentPosition.x);
        Logger.getInstance().recordOutput(getPrefix() + "CurrentZPosition", currentPosition.y);

        upperArm.periodic();
        lowerArm.periodic();

        // Update the mechanism for visualization
        var currentAngles = getCurrentValue();
        realLowerArm.setAngle(currentAngles.x);
        // We use slightly different frames of reference compared to WPI, so we need to rotate this half a turn.
